...
class MiBand3(Peripheral):
    ...
    def dfuUpdate(self, fileName):
        ...
        input('Press Enter to Continue')
    def start_raw_data_realtime(self, heart_measure_callback=None, heart_raw_callback=None, accel_raw_callback=None, heartRate=None):
            char_m = self.svc_heart.getCharacteristics(UUIDS.CHARACTERISTIC_HEART_RATE_MEASURE)[0]
            char_d = char_m.getDescriptors(forUUID=UUIDS.NOTIFICATION_DESCRIPTOR)[0]
            ...
            t = time.time()
            while True:
                if heartRate.breakFlag == True:
                    return                
                self.waitForNotifications(0.5)            
                self._parse_queue()	#retrieve data to the callback
